...
async def handle_command(websocket, text: str):
    ...
    # /buatroom <nama> [password]
        ...
        print(f"[{timestamp()}] Room '{rname}' dibuat oleh {username}")
    ...
    # /deleteroom
        ...
        del rooms[rname]
        await broadcast_online("global")
        print(f"[{timestamp()}] Room '{rname}' dihapus oleh {username}")
    ...
    # /w <username> <message> — Whisper
        ...
        print(f"[{timestamp()}] [WHISPER] {username} → {target_info['username']}: {whisper_text}")
...
async def handler(websocket):
    ...
    try:
        async for raw in websocket:
            ...
            if msg_type == "message":
                ...
                if text.startswith("/"):
                    ...
                    print(f"[{timestamp()}] [{room}] {username}: {text}")
                    msg_data = {
                        "type": "message",
                        "from": username,
                        "text": text,
        ...
        if websocket in clients:
            ...
            print(f"[{timestamp()}] - {username} left")
...
async def clear_history_loop():
    ...
    while True:
        ...
        if elapsed >= HISTORY_TTL_HOURS * 3600:
            ...
            print(f"[{timestamp()}] 🗑 Global history cleared (24h reset)")
...
async def main():
    print("Chat Server berjalan di ws://0.0.0.0:8765")
    print(f"History: max {MAX_HISTORY} pesan, reset setiap {HISTORY_TTL_HOURS} jam")
    print("-" * 40)
